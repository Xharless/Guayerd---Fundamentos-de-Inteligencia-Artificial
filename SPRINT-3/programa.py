...
def mostrar_menu(datos_cargados=False, analisis_cargado=False):
    ...
    print("\n" + "="*90)
    print("║" + " "*88 + "║")
    print("║" + "SISTEMA DE CONTROL DE STOCK Y VENTAS - CON MACHINE LEARNING".center(88) + "║")
    print("║" + " "*88 + "║")
    print("="*90)
    ...
    print("\n  📊 ANALISIS SPRINT-2 (DATOS Y EXPLORACION)")
    if datos_cargados:
    ...
    print("\n  🤖 MACHINE LEARNING SPRINT-3 (NUEVO)")
    if datos_cargados:
        print(f"    {len(documentacion) + 11}. 🤖 Entrenar modelo de predicción de ingresos")
    else:
        print(f"    {len(documentacion) + 11}. 🤖 Entrenar modelo [No disponible]")
    
    print("\n  ⚙️  EXTRAS")
    print(f"    {len(documentacion) + 12}. 🔄 Ver diagrama de flujo")
...
def mostrar_seccion(opcion, datos=None, df_analisis=None):
    ...
    if opcion == num_doc + 11:
        if datos:
            ejecutar_machine_learning(datos[0], datos[1], datos[2])
        return
    
    # Diagrama de flujo
    if opcion == num_doc + 12:
...
def mostrar_pantalla_carga():
    ...
    print("\n" + "="*90)
    print(" "*25 + "⏳ CARGANDO SISTEMA DE CONTROL DE STOCK Y VENTAS ⏳")
    print("="*90)
